...
func (c *Client) ChatWithJSON(ctx context.Context, messages []Message) (string, error) {
	// 添加JSON格式要求到系统消息
	jsonSystemMessage := Message{
		Role:    "system",
		Content: "请确保你的回复是有效的JSON格式，不要包含任何额外的文字或格式标记如```json等。直接返回纯JSON内容。",
	}

	// 将JSON格式要求插入到消息列表开头
	allMessages := append([]Message{jsonSystemMessage}, messages...)
