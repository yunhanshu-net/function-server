...
import (
	...
	"github.com/yunhanshu-net/function-server/pkg/qwen"
	"github.com/yunhanshu-net/pkg/logger"
	"github.com/yunhanshu-net/pkg/x/httpx"
)

// FunctionGenQwenV2 基于独立千问包的函数生成服务
type FunctionGenQwenV2 struct {
	qwenClient  *qwen.Client
	serviceTree *ServiceTree
}

// NewFunctionGenQwenV2 创建千问函数生成服务V2
func NewFunctionGenQwenV2() *FunctionGenQwenV2 {
	// 千问配置
	config := &qwen.Config{
	...
	return &FunctionGenQwenV2{
		qwenClient:  qwen.NewClient(config),
		serviceTree: NewServiceTree(db.GetDB()),
	}
}

// QwenAICodeResponseV2 千问AI代码响应结构体V2
type QwenAICodeResponseV2 struct {
...
// FunctionGenWithQwenV2 使用千问生成函数代码V2
func (s *FunctionGenQwenV2) FunctionGenWithQwenV2(ctx context.Context, req *dto.FunctionGenReq) (*model.FunctionGen, error) {
	if ctx.Err() != nil {
		return nil, ctx.Err()
	}

	var aiResp QwenAICodeResponseV2
	var ragResp RagResp
	...
	task := func() error {
		...
		// 准备知识库消息 - 转换为qwen.Message格式
		ragMessages := ragResp.DecodeData()
		knowledgeMessages := s.convertLLMToQwenMessages(ragMessages)

		// 添加用户请求
		qwenMessages := append(knowledgeMessages, qwen.Message{
			Role:    "user",
			Content: fmt.Sprintf("<message>%s</message>", req.Message+contextInfo),
		})

		// 调用千问生成代码
		err = s.generateCodeWithQwenV2(ctx, qwenMessages, &aiResp)
		cost := time.Since(now)

		if err != nil {
			logger.Infof(ctx, "千问V2函数生成失败 req：%s： err:%s cost：%s", req.Message, err.Error(), cost)
			return err
		}

		logger.Infof(ctx, "千问V2函数生成成功 req：%s：cost：%s", req.Message, cost)
		...
		for retry := 0; retry <= maxRetries; retry++ {
			...
			err = s.createRunnerFuncV2(ctx, rf)
			if err != nil {
				lastError = err
				// 检查是否是编译失败错误
				if strings.Contains(err.Error(), "go build failed") && retry < maxRetries {
					...
					// 使用千问进行代码修正
					fixedCode, fixErr := s.fixCodeWithQwenV2(ctx, aiResp.Code, err.Error(), qwenMessages)
					if fixErr != nil {
	...
	if req.Async {
		go func() {
			err = task()
			if err != nil {
				logger.Errorf(ctx, "千问V2函数生成任务失败：%s", err.Error())
			}
		}()
		return fg, nil
	} else {
...
// convertLLMToQwenMessages 转换LLM消息格式为qwen.Message
func (s *FunctionGenQwenV2) convertLLMToQwenMessages(messages interface{}) []qwen.Message {
	var qwenMessages []qwen.Message

	// 根据实际的LLM消息类型进行转换
	switch msgs := messages.(type) {
	case []map[string]interface{}:
		for _, msg := range msgs {
			qwenMsg := qwen.Message{
	...
	default:
		// 如果是其他类型，尝试通过反射或类型断言处理
		// 这里先返回空切片，避免编译错误
		logger.Debugf(context.Background(), "未知的消息类型: %T", messages)
	}
...
// generateCodeWithQwenV2 使用千问生成代码V2
func (s *FunctionGenQwenV2) generateCodeWithQwenV2(ctx context.Context, messages []qwen.Message, response *QwenAICodeResponseV2) error {
	// 添加结构化输出要求
	systemPrompt := qwen.Message{
		Role: "system",
...
7. 可以直接编译和运行`,
	...
	// 将系统提示添加到消息开头
	allMessages := append([]qwen.Message{systemPrompt}, messages...)

	// 调用千问API
	jsonContent, err := s.qwenClient.ChatWithJSON(ctx, allMessages)
	if err != nil {
		return fmt.Errorf("千问V2 API调用失败: %w", err)
	}
	...
	logger.Debugf(ctx, "千问V2清理后的JSON响应: %s", jsonContent)

	// 解析JSON响应
	if err := json.Unmarshal([]byte(jsonContent), response); err != nil {
		return fmt.Errorf("解析千问V2 JSON响应失败: %w, 内容: %s", err, jsonContent)
	}

	// 验证必要字段
	if response.Code == "" {
		return fmt.Errorf("千问V2返回的代码为空")
	}
	if response.EnName == "" {
		return fmt.Errorf("千问V2返回的英文函数名为空")
	}

	return nil
}

// fixCodeWithQwenV2 使用千问修正编译失败的代码V2
func (s *FunctionGenQwenV2) fixCodeWithQwenV2(ctx context.Context, originalCode, errorMsg string, knowledgeMessages []qwen.Message) (string, error) {
	// 构建修正提示
...
请以JSON格式返回：
{
  "code": "修正后的完整代码"
	...
	// 组合消息：知识库 + 修正提示
	messages := append(knowledgeMessages, qwen.Message{
		Role:    "user",
		Content: fixPrompt,
	})
	...
	if err != nil {
		return "", fmt.Errorf("千问V2代码修正调用失败: %w", err)
	}
	...
	if err := json.Unmarshal([]byte(jsonContent), &fixResp); err != nil {
		return "", fmt.Errorf("解析千问V2修正响应失败: %w, 内容: %s", err, jsonContent)
	}

	fixedCode := fixResp.Code
	if fixedCode == "" {
		return "", fmt.Errorf("千问V2返回空的修正代码")
	}

	return fixedCode, nil
}

// createRunnerFuncV2 创建RunnerFunc（模拟方法，实际需要调用真实的service）
func (s *FunctionGenQwenV2) createRunnerFuncV2(ctx context.Context, rf *model.RunnerFunc) error {
	// 这里应该调用真实的RunnerFunc服务的Create方法
	// 暂时模拟编译成功
	logger.Infof(ctx, "模拟创建RunnerFunc V2: %s", rf.Name)
	return nil
}
